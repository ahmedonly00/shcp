...
class PrescriptionServiceTest {
    ...
    void getById_shouldThrowForbidden_forUnrelatedUser() {
        ...
        assertThatThrownBy(() ->
                prescriptionService.getById(p.getPrescriptionId(), UUID.randomUUID(), Role.PATIENT))
                .isInstanceOf(AppException.class)
    ...
    private Patient buildPatient(UUID userId) {
        User u = new User();
        u.setUserId(userId); u.setName("Alice"); u.setEmail("a@test.com");
        u.setRole(Role.PATIENT); u.setVerified(true);
        Patient p = new Patient();
        p.setUserId(userId); p.setUser(u);
        return p;
    }

    private Provider buildProvider(UUID userId) {
        User u = new User();
        u.setUserId(userId); u.setName("Dr Test"); u.setEmail("dr@test.com");
        u.setRole(Role.PROVIDER); u.setVerified(true);
        Provider pv = new Provider();
        pv.setUserId(userId); pv.setUser(u);
        pv.setActive(true); pv.setRating(BigDecimal.valueOf(4.5));
        return pv;
    }
